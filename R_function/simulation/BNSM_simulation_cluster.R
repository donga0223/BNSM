...
n.group <- 3  ## generate data with three group (small false positive, small false negative, both are small)
myemp <- c(1,9,1,9,9,1)
myepp <- c(1,9,9,1,1,9)


true_np <- 0.05  ## true network density
mymodel_np <- "obs" 
...
## generate data with m relations and V nodes 
...
for(i in 1:length(a1.em.list)){
  ...
  save.image(paste("simulation/BNSM_simulation_rep_", k, ".RData", sep = ""))
