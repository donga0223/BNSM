...
for(school in schools){
  ...
  mycol <- c("red", brewer.pal(5, "Set2")[c(1,4,5)])
  mylegend <- c("True" , "M1", "M4", "M5")
  ...
  legend(legendlocation, legend = mylegend, col = mycol, lwd = 3, cex = 2)
  ...
  collec_all_emep_xy <- function(mydensity, iter){
    e_xmatrix <- e_ymatrix <- matrix(NA, nrow = iter, ncol = length(mydensity[[1]]$x))
    for(i in 1:iter){
      e_xmatrix[i,] <- mydensity[[i]]$x
      e_ymatrix[i,] <- mydensity[[i]]$y
    }
    
    ex_summary <- apply(e_xmatrix, 2, quantile, c(0.025, 0.5, 0.975))
    ey_summary <- apply(e_ymatrix, 2, quantile, c(0.025, 0.5, 0.975))
    
    return(list(ex_summary = ex_summary, ey_summary = ey_summary))
  } 

  theta1_em_summary <- collec_all_emep_xy(theta1.em.density, iter)
  theta2_em_summary <- collec_all_emep_xy(theta2.em.density, iter)
  theta3_em_summary <- collec_all_emep_xy(theta3.em.density, iter)
  #theta4_em_summary <- collec_all_emep_xy(theta4.em.density, iter)
  #theta5_em_summary <- collec_all_emep_xy(theta5.em.density, iter)

  theta1_ep_summary <- collec_all_emep_xy(theta1.ep.density, iter)
  theta2_ep_summary <- collec_all_emep_xy(theta2.ep.density, iter)
  theta3_ep_summary <- collec_all_emep_xy(theta3.ep.density, iter)
  #theta4_ep_summary <- collec_all_emep_xy(theta4.ep.density, iter)
  #theta5_ep_summary <- collec_all_emep_xy(theta5.ep.density, iter)



  plot(xs, emy, type = "l", lwd = 3, col = 2, xlab = "e-", ylab = "Density", main = paste(simname, " Posterior for all observations \n in 50 replicates", sep = ""), cex.main = 2, cex.lab = 2, cex.axis = 2)
  
  lines(theta1_em_summary$ex_summary[1,], theta1_em_summary$ey_summary[1,], col = mycol[2], lty = 2)
  lines(theta1_em_summary$ex_summary[3,], theta1_em_summary$ey_summary[3,], col = mycol[2], lty = 2)
  polygon(c(theta1_em_summary$ex_summary[1,], rev(theta1_em_summary$ex_summary[3,])), c(theta1_em_summary$ey_summary[1,], rev(theta1_em_summary$ey_summary[3,])), col = adjustcolor(mycol[2], alpha.f = 0.3), border = NA)
  lines(theta1_em_summary$ex_summary[2,], theta1_em_summary$ey_summary[2,], col = mycol[2], lwd = 2)

  lines(theta2_em_summary$ex_summary[1,], theta2_em_summary$ey_summary[1,], col = mycol[3], lty = 2)
  lines(theta2_em_summary$ex_summary[3,], theta2_em_summary$ey_summary[3,], col = mycol[3], lty = 2)
  polygon(c(theta2_em_summary$ex_summary[1,], rev(theta2_em_summary$ex_summary[3,])), c(theta2_em_summary$ey_summary[1,], rev(theta2_em_summary$ey_summary[3,])), col = adjustcolor(mycol[3], alpha.f = 0.3), border = NA)
  lines(theta2_em_summary$ex_summary[2,], theta2_em_summary$ey_summary[2,], col = mycol[3], lwd = 2)

  lines(theta3_em_summary$ex_summary[1,], theta3_em_summary$ey_summary[1,], col = mycol[4], lty = 2)
  lines(theta3_em_summary$ex_summary[3,], theta3_em_summary$ey_summary[3,], col = mycol[4], lty = 2)
  polygon(c(theta3_em_summary$ex_summary[1,], rev(theta3_em_summary$ex_summary[3,])), c(theta3_em_summary$ey_summary[1,], rev(theta3_em_summary$ey_summary[3,])), col = adjustcolor(mycol[4], alpha.f = 0.3), border = NA)
  lines(theta3_em_summary$ex_summary[2,], theta3_em_summary$ey_summary[2,], col = mycol[4], lwd = 2)
  
  lines(xs, emy, type = "l", lwd = 3, col = 2)
  legend(legendlocation, bg = 'white', legend = mylegend, col = mycol, lwd = 3, cex = 2)
  
  
  plot(xs, epy, type = "l", lwd = 3, col = 2, xlab = "e+", ylab = "Density", main = paste(simname, " Posterior for all observations \n in 50 replicates", sep = ""), cex.main = 2, cex.lab = 2, cex.axis = 2)

  lines(theta1_ep_summary$ex_summary[1,], theta1_ep_summary$ey_summary[1,], col = mycol[2], lty = 2)
  lines(theta1_ep_summary$ex_summary[3,], theta1_ep_summary$ey_summary[3,], col = mycol[2], lty = 2)
  polygon(c(theta1_ep_summary$ex_summary[1,], rev(theta1_ep_summary$ex_summary[3,])), c(theta1_ep_summary$ey_summary[1,], rev(theta1_ep_summary$ey_summary[3,])), col = adjustcolor(mycol[2], alpha.f = 0.3), border = NA)
  lines(theta1_ep_summary$ex_summary[2,], theta1_ep_summary$ey_summary[2,], col = mycol[2], lwd = 2)

  lines(theta2_ep_summary$ex_summary[1,], theta2_ep_summary$ey_summary[1,], col = mycol[3], lty = 2)
  lines(theta2_ep_summary$ex_summary[3,], theta2_ep_summary$ey_summary[3,], col = mycol[3], lty = 2)
  polygon(c(theta2_ep_summary$ex_summary[1,], rev(theta2_ep_summary$ex_summary[3,])), c(theta2_ep_summary$ey_summary[1,], rev(theta2_ep_summary$ey_summary[3,])), col = adjustcolor(mycol[3], alpha.f = 0.3), border = NA)
  lines(theta2_ep_summary$ex_summary[2,], theta2_ep_summary$ey_summary[2,], col = mycol[3], lwd = 2)

  lines(theta3_ep_summary$ex_summary[1,], theta3_ep_summary$ey_summary[1,], col = mycol[4], lty = 2)
  lines(theta3_ep_summary$ex_summary[3,], theta3_ep_summary$ey_summary[3,], col = mycol[4], lty = 2)
  polygon(c(theta3_ep_summary$ex_summary[1,], rev(theta3_ep_summary$ex_summary[3,])), c(theta3_ep_summary$ey_summary[1,], rev(theta3_ep_summary$ey_summary[3,])), col = adjustcolor(mycol[4], alpha.f = 0.3), border = NA)
  lines(theta3_ep_summary$ex_summary[2,], theta3_ep_summary$ey_summary[2,], col = mycol[4], lwd = 2)

  lines(xs, epy, type = "l", lwd = 3, col = 2)
  legend("topright", bg = 'white', legend = mylegend, col = mycol, lwd = 3, cex = 2)
  ...
  boxplot(npbox[,c(seq(1,t.n,6),seq(2,t.n,6),seq(3,t.n,6),seq(4,t.n,6),seq(5,t.n,6),seq(6,t.n,6))], col = mycol[2:(mn+1)]
          , main= paste(simname, " Posterior mean for Theta", sep=""), 
          cex.main = 2, cex.lab = 2, xaxt = "n", cex.axis = 2, ylab = 'Network Density',
          ylim = c(min(npbox)-0.025, max(npbox)+0.02))
  ...
  legend("bottomright", bg = 'white', legend = "True mean density", lwd = 3, cex = 1.5, col = 2)
  legend("topleft", bg = 'white', legend = mylegend[2:4], col = mycol[2:4], pch = 19, cex = 1.5, horiz = TRUE)
  
  ### boxplot for the theta with only mean values
  npbox <- cbind(mean.np1, mean.np2, mean.np3)
  boxplot(npbox[,c(seq(1,t.n,6),seq(2,t.n,6),seq(3,t.n,6),seq(4,t.n,6),seq(5,t.n,6),seq(6,t.n,6))],
          col = mycol[2:(mn+1)], main= paste(simname, "\n Posterior mean for Theta in 50 replicates", sep=""),  #main = expression(paste("Posterior for ", theta, " in 50 replicates")), 
          cex.main = 2, cex.lab = 2, cex.axis = 2, xaxt = "n", ylab = 'Network Density',
          ylim = c(min(npbox)-0.025, max(npbox)+0.02))
  abline(v = (1:m)*mn+0.5, lty = 2)
  axis(side=1, at=(1:m)*mn-mn/2, mgp=c(0,0.5,0), labels = c("f2f", "online", "eatout", "pa", "screen", "lunch"), cex.axis = 1.3, tick = 0)
  for (i in 1:V)  lines(c(mn*(i-1)+0.5, mn*i+0.5), rep(true_np[i],2), col = 2, lwd = 2)
  ...
  ROC.rep.theta1.x <- ROC.rep.theta2.x <- ROC.rep.theta3.x <- ROC.rep.theta4.x <- ROC.rep.theta5.x <- matrix(NA, ncol = length(myinteger), nrow = myrep)
  ROC.rep.theta1.y <- ROC.rep.theta2.y <- ROC.rep.theta3.y <- ROC.rep.theta4.y <- ROC.rep.theta5.y <- matrix(NA, ncol = length(myinteger), nrow = myrep)

  ROC.rep <- function(theta.perf){
  
    mylocation <- c()
    ROC.rep.theta.x <- matrix(NA, ncol = length(myinteger), nrow = myrep)
    ROC.rep.theta.y <- matrix(NA, ncol = length(myinteger), nrow = myrep)

    for(i in 1:myrep){
      for(j in 1:length(myinteger)){
        mylocation[j] <- max(which((theta.perf[[i]]@alpha.values)[[1]]>=myinteger[j]))
      }
    ROC.rep.theta.x[i,] <- as.data.frame(theta.perf[[i]]@x.values)[,1][mylocation]
    ROC.rep.theta.y[i,] <- as.data.frame(theta.perf[[i]]@y.values)[,1][mylocation]
    }
    return(list(ROC.rep.theta.x = ROC.rep.theta.x, ROC.rep.theta.y = ROC.rep.theta.y))
  }
  ROC.rep.theta1.x <- ROC.rep(theta1.perf)$ROC.rep.theta.x
  ROC.rep.theta1.y <- ROC.rep(theta1.perf)$ROC.rep.theta.y

  ROC.rep.theta2.x <- ROC.rep(theta2.perf)$ROC.rep.theta.x
  ROC.rep.theta2.y <- ROC.rep(theta2.perf)$ROC.rep.theta.y

  ROC.rep.theta3.x <- ROC.rep(theta3.perf)$ROC.rep.theta.x
  ROC.rep.theta3.y <- ROC.rep(theta3.perf)$ROC.rep.theta.y

  res <- res1 <- c()
  for(i in 1:myrep){
    res[i] <- ifelse(ROC.rep.theta1.y[i,min(which(ROC.rep.theta1.x[i,] < uni.inter.xy.table[i,1]))]<uni.inter.xy.table[i,2],1,0)
    res1[i] <- ifelse(ROC.rep.theta1.y[i,max(which(ROC.rep.theta1.x[i,] > uni.inter.xy.table[i,1]))]<uni.inter.xy.table[i,2],1,0)
  ...
  roc2.obsmean <- roc.threshold(theta2.b01, g10, threshold = obs.g10)
  roc2.estmean <- roc.threshold(theta2.b01, g10, threshold = theta2.b01)

  roc3.obsmean <- roc.threshold(theta3.b01, g10, threshold = obs.g10)
  roc3.estmean <- roc.threshold(theta3.b01, g10, threshold = theta3.b01)

  
  par(mar = c(5, 5, 4, 2) + 0.1) 
  plot(apply(ROC.rep.theta1.x,2,mean, na.rm = TRUE), apply(ROC.rep.theta1.y,2,mean, na.rm = TRUE), 
    col = mycol[2], main = paste("S-", school," ROC curve in 50 replicates", sep = " "),
    cex.main = 2, cex.lab = 2, cex.axis = 2, type = "l",
    xlab = "False positive rate", ylab = "True Positive rate")
  lines(apply(ROC.rep.theta2.x,2,mean, na.rm = TRUE), apply(ROC.rep.theta2.y,2,mean, na.rm = TRUE), 
    type = "l", col = mycol[3])
  lines(apply(ROC.rep.theta3.x,2,mean, na.rm = TRUE), apply(ROC.rep.theta3.y,2,mean, na.rm = TRUE), 
    type = "l", col = mycol[4])
  ...
  points(mean(roc1.obsmean[,1]), mean(roc1.obsmean[,2]), pch = 17, col = mycol[2], cex = 2)
  points(mean(roc1.estmean[,1]), mean(roc1.estmean[,2]), pch = 8, col = mycol[2], cex = 2)
  points(mean(roc2.obsmean[,1]), mean(roc2.obsmean[,2]), pch = 17, col = mycol[3], cex = 2)
  points(mean(roc2.estmean[,1]), mean(roc2.estmean[,2]), pch = 8, col = mycol[3], cex = 2)
  points(mean(roc3.obsmean[,1]), mean(roc3.obsmean[,2]), pch = 17, col = mycol[4], cex = 2)
  points(mean(roc3.estmean[,1]), mean(roc3.estmean[,2]), pch = 8, col = mycol[4], cex = 2)
  
  legend("bottomright", pch = c(16,15,17,8,NA,NA,NA), lty = c(NA,NA,1,1,1,1,1), 
    lwd = 2, col = c(2,2,1,1,mycol[-1]), cex = 1.8, bg = 'white', 
    legend = c("Union", "Intersection", expression(t == bar(Y)), expression(t == bar(hat(theta))), "M1 mean(ROC)" , "M4 mean(ROC)" , "M5 mean(ROC)"))
  ...
  if(2==3){
              ...
              plot(theta2.perf[[1]], col = mycol[3], add = TRUE, cex.axis = 2)
              plot(theta3.perf[[1]], col = mycol[4], add = TRUE, cex.axis = 2)
              #plot(theta4.perf[[1]], col = mycol[5], add = TRUE)
              
              for(i in 2:myrep){
                plot(theta1.perf[[i]], col = mycol[2], add = TRUE, cex.axis = 2)
                plot(theta2.perf[[i]], col = mycol[3], add = TRUE, cex.axis = 2)
                plot(theta3.perf[[i]], col = mycol[4], add = TRUE, cex.axis = 2)
  ...
  boxplot(npbox[,c(seq(1,t.n,6),seq(2,t.n,6),seq(3,t.n,6),seq(4,t.n,6),seq(5,t.n,6),seq(6,t.n,6))]
          , col = mycol[2:(mn+1)], main = paste(simname, " Posterior predictive check", sep=""), 
          cex.main = 2, cex.lab = 2, xaxt = "n", cex.axis = 2, ylab = 'Network Density',
          ylim = c(min(npbox)-0.025, max(npbox)+0.02))
  
  abline(v = (1:m)*mn+0.5, lty = 2)
  axis(side=1, at=(1:m)*mn-mn/2, mgp=c(0,0.5,0), labels = c("f2f", "online", "eatout", "pa", "screen", "lunch"), 
       cex.axis = 1.3, tick = 0)
  ...
  legend("bottomrigh", bg = 'white', legend = "Observed mean density", lwd = 3, cex = 1.5, col = 2)
  legend("topleft", bg = 'white', legend = mylegend[2:4], col = mycol[2:4], pch = 19, cex = 1.5, horiz = TRUE)
  
  ### boxplot for the posterior predictive check with only mean values
...
boxplot(npbox[,c(seq(1,t.n,6),seq(2,t.n,6),seq(3,t.n,6),seq(4,t.n,6),seq(5,t.n,6),seq(6,t.n,6))], col = mycol
  , main = paste(simname, "\n Posterior mean for Theta in 50 replicates", sep = ""), 
  cex.main = 2, cex.lab = 2, xaxt = "n", ylab = 'Network Density',
  ylim = c(min(npbox)-0.025, max(npbox)+0.02))
abline(v = (1:m)*mn+0.5, lty = 2)
axis(side=1, at=(1:m)*mn-mn/2, mgp=c(0,0.5,0), labels = c("f2f", "online", "eatout", "pa", "screen", "lunch"), 
    cex.axis = 1.3, cex.lab = 2, tick = 0)
#axis(side=1, at = 1:(mn*m), mgp=c(0,1.5,0), labels = rep(c("B",paste0("m",c(1,4,5))),6), tick = 0, cex.axis = 0.5)
for (i in 1:V)  lines(c(mn*(i-1)+0.5, mn*i+0.5), rep(true_np[i],2), col = 2, lwd = 2)
legend("bottomrigh", bg = 'white', legend = "True mean density", lwd = 3, cex = 1.5, col = 2)
...
#npbox <- cbind(mean.pred.y1, mean.pred.y2, mean.pred.y3
boxplot(npbox[,c(seq(1,t.n,6),seq(2,t.n,6),seq(3,t.n,6),seq(4,t.n,6),seq(5,t.n,6),seq(6,t.n,6))], col = mycol
        , main = paste(simname, ", Posterior Predictive check", sep = ""), 
        cex.main = 2, cex.lab = 2, xaxt = "n", ylab = 'Network Density',
        ylim = c(min(npbox)-0.025, max(npbox)+0.02))
abline(v = (1:m)*mn+0.5, lty = 2)
axis(side=1, at=(1:m)*mn-mn/2, mgp=c(0,0.5,0), labels = c("f2f", "online", "eatout", "pa", "screen", "lunch"), 
